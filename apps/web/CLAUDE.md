...
**HeroUI's `Alert` renders a plain `<div>` with no `role="alert"`/`aria-live` built in** (confirmed via `get_source.mjs Alert`) — add `role="alert"` (and `aria-live="polite"` if it can appear without a page navigation, e.g. an inline form error) explicitly whenever `Alert` communicates an error/status change, the same way plain `<p>` error messages elsewhere in this app already carry `role="alert"`. Don't assume a HeroUI component ships the accessible semantics its name implies — check the source before shipping a new usage.
...
`src/lib/api.ts` additionally exports: `getMeetingById(accessToken, id)` (`GET /meetings/:id`); the `MeetingFile`/`MeetingFileKind`/`MeetingFileStatus` types plus `getMeetingFiles(accessToken, meetingId)` (`GET /meetings/:id/files`) and `getMeetingFileContentUrl(meetingId, fileId)` (builds the `GET .../content` URL — Phase 2 doesn't fetch this directly, a future phase's player will); and `uploadMeetingFile(accessToken, meetingId, file, kind, onProgress?)`, the one function here that uses `XMLHttpRequest` instead of `fetch` — `fetch` has no upload-progress event, so this is the deliberate exception to the "plain `fetch`" pattern, still normalizing failures into the same `ApiError` shape (`onerror` → a synthetic network-error message, non-2xx → the parsed JSON body) so callers don't need to know it's implemented differently.

`src/lib/meeting-format.ts` centralizes cross-page meeting-status display, so it isn't reintroduced/duplicated whenever a new page needs it: `STATUS_LABELS`/`STATUS_COLORS` (Russian labels + HeroUI `Chip` `color` per `MeetingStatus`), `formatDate()`, and `pluralize(count, one, few, many)` (Russian plural forms — `count % 10`/`% 100` rules — used for "N участник/участника/участников"; reach for this anytime a count needs a Russian noun, not just for participants). `src/app/page.tsx` and `src/app/meetings/[id]/page.tsx` both import from here rather than each declaring their own copy.

`src/components/` holds cross-page presentational pieces (first introduced for Phase 2): `MeetingFileUploadForm.tsx` (kind `RadioGroup` + hidden `<input type="file">` triggered by a visible button + `ProgressBar` + error `Alert`) and `MeetingFilesList.tsx` (self-polls `getMeetingFiles` on a `setInterval`, independent of its parent — a parent triggers an out-of-band refresh by bumping a `refreshSignal` number prop, which the list's effect has in its dependency array). Before this, every page rendered its own JSX inline; extract into `src/components/` when a piece is reused across pages or complex enough to clutter its page, following the plain-function-component style already established (no shared UI kit/wrapper layer beyond HeroUI itself).

`src/app/meetings/[id]/page.tsx` (`/meetings/:id`) is the second auth-gated page, following `src/app/page.tsx`'s exact gating shape (`useState`+`useEffect` reading `localStorage`, not `useSyncExternalStore` — see the gotchas below). It fetches the meeting via `getMeetingById`, renders title/date/status/participant count (host + `participants.length`, since there's no `/users` listing endpoint to resolve participant ids to emails yet — a count is what's feasible client-side today), then `MeetingFileUploadForm` and `MeetingFilesList` in `Card`s. Its own `layout.tsx` sets a static `title: 'Встреча'` (not dynamic per-meeting — the page itself is a client component, so per-meeting `<title>` would need `generateMetadata` in a server component wrapper, not attempted for this phase).

`src/app/page.tsx`'s meeting list items are `next/link` `<Link href={`/meetings/${meeting.id}`}>` wrapping the whole row (not a HeroUI `Link` — plain `next/link` avoids the `render`-prop typing issue noted below entirely, since there's no HeroUI component involved).
